...
export const statisticItemSchema = z.object({
  ...
  changePercentage: z.number().optional().openapi({ description: "Percentage change from previous period" }),
}).openapi({ description: "Individual statistic item" });

// Time-based statistic schema
export const timeBasedStatisticSchema = z.object({
...
export const libraryStatisticsSchema = z.object({
  ...
  booksBySubject: z.array(z.object({
...
export const userStatisticsSchema = z.object({
  ...
  favoriteGenres: z.array(z.object({
    subjectId: z.string().uuid(),
    subjectName: z.string(),
    borrowCount: z.number(),
  })).openapi({ description: "User's favorite genres by borrow count" }),
  favoriteAuthors: z.array(z.object({
  ...
  completionRate: z.number().min(0).max(100).optional().openapi({ description: "Book completion rate percentage" }),
...
// Book statistics
export const bookStatisticsSchema = z.object({
  ...
  libraryAvailability: z.array(z.object({
    libraryId: z.string().uuid(),
    libraryName: z.string(),
...
// Author statistics
export const authorStatisticsSchema = z.object({
  ...
  averageRating: z.number().min(0).max(5).optional().openapi({ description: "Average rating across all books" }),
  popularBooks: z.array(z.object({
    bookId: z.string().uuid(),
    title: z.string(),
    borrowCount: z.number(),
  ...
  publishingTimeline: z.array(z.object({
    year: z.number(),
    bookCount: z.number(),
    titles: z.array(z.string()).optional(),
  })).openapi({ description: "Publishing activity by year" }),
  genreDistribution: z.array(z.object({
    subjectId: z.string().uuid(),
    subjectName: z.string(),
    bookCount: z.number(),
...
export const analyticsDashboardSchema = z.object({
  ...
  topPerformers: z.object({
    ...
    activeLibraries: z.array(z.object({
      id: z.string(),
      name: z.string(),
      memberCount: z.number(),
      borrowCount: z.number(),
...
export const statisticalSummarySchema = z.object({
  ...
  percentiles: z.object({
    ...
    p90: z.number(),
    p95: z.number(),
  }).optional().openapi({ description: "Percentile values" }),
