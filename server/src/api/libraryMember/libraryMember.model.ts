...
export const libraryMemberSchema = z.object({
  ...
  role: z.enum(["owner", "manager", "staff", "member"]).default("member").openapi({ description: "Role of the member in the library" }),
  permissions: z.array(z.string()).nullable().optional().openapi({ description: "Array of permission strings" }),
  ...
  invitedBy: idSchema.nullable().optional().openapi({ description: "ID of the user who invited this member" }),
...
// Response schemas
export const libraryMemberArraySchema = z.array(libraryMemberSchema).openapi({ description: "Array of library members" });

// Library member with detailed information (business logic layer)
export const libraryMemberWithDetailsSchema = libraryMemberSchema.extend({
  user: z.object({
    id: idSchema,
    username: z.string(),
    firstName: z.string(),
  ...
  inviter: z.object({
    ...
    firstName: z.string(),
    lastName: z.string(),
  }).nullable().optional().openapi({ description: "User who invited this member" }),
}).openapi({ description: "Library member with detailed information" });

// Library member with activity stats (business logic layer)
export const libraryMemberWithStatsSchema = libraryMemberWithDetailsSchema.extend({
  ...
  averageBorrowDuration: z.number().min(0).optional().openapi({ description: "Average borrow duration in days" }),
