...
export const bookSchema = z.object({
  id: idSchema,
  title: z.string().min(2, "Title must be at least 2 characters long").max(200, "Title cannot exceed 200 characters").openapi({ description: "Book title" }),
  ...
  datePublished: z.date().max(new Date(), "Publication date cannot be in the future").nullable().optional().openapi({ description: "Publication date" }),
...
export const createBookSchema = z.object({
  title: titleSchema,
  author: z.string().min(2, "Author name must be at least 2 characters long").max(100, "Author name cannot exceed 100 characters").openapi({ description: "Author name (will be linked to existing author or create new)" }),
  publisher: z.string().min(1, "Publisher name is required").max(100, "Publisher name cannot exceed 100 characters").openapi({ description: "Publisher name (will be linked to existing publisher or create new)" }),
...
export const createBookWithIdsSchema = z.object({
  title: z.string().min(2, "Title must be at least 2 characters long").max(200, "Title cannot exceed 200 characters").openapi({ description: "Book title" }),
  ...
  datePublished: z.date().max(new Date(), "Publication date cannot be in the future").optional().openapi({ description: "Publication date" }),
...
export const createBooksBulkSchema = z.array(createBookWithIdsSchema).min(1).max(100).openapi({ description: "Array of books for bulk creation" });
...
export const bookWithRelationsSchema = bookSchema.extend({
  ...
  }).nullable().optional().openapi({ description: "Subject information" }),
}).openapi({ description: "Book with related entity details" });

// Book with computed statistics (business logic layer)
export const bookWithStatsSchema = bookWithRelationsSchema.extend({
  ...
  averageRating: z.number().min(0).max(5).nullable().optional().openapi({ description: "Average user rating" }),
  popularInLibraries: z.array(z.object({
    libraryId: idSchema,
    libraryName: z.string(),
    borrowCount: z.number().int().min(0),
  })).optional().openapi({ description: "Libraries where this book is most popular" }),
