...
export const libraryStatsSchema = z.object({
  ...
  popularBooks: z.array(
    bookSchema.extend({
      borrowCount: z.number().int().min(0).openapi({ description: "Number of times this book has been borrowed" }),
    })
...
export const userStatsSchema = z.object({
  totalBooksRead: z.number().int().min(0).openapi({ description: "Total number of books the user has read" }),
  currentlyReading: z.number().int().min(0).openapi({ description: "Number of books currently being read" }),
  ...
  librariesJoined: z.number().int().min(0).openapi({ description: "Number of libraries the user is a member of" }),
