...
export const createBookRequestSchema = z
  .object({
    ...
    authorName: z
      ...
      .openapi({ description: "Author name (will be linked to Author entity)" }),
    publisherName: z
      .string()
      .min(1, "Publisher name is required")
      .max(100, "Publisher name cannot exceed 100 characters")
      .openapi({ description: "Publisher name (will be linked to Publisher entity)" }),
    subjectName: z
      .string()
      .max(100)
      .optional()
      .openapi({ description: "Subject name (will be linked to Subject entity)" }),
...
export const addBookToLibraryRequestSchema = z
  .object({
    ...
    quantity: z
      ...
      .max(1000)
      .default(1)
      .openapi({ description: "Number of copies to add" }),
...
export const removeBookFromLibraryRequestSchema = z
  .object({
...
export const updateLibraryBookRequestSchema = z
  .object({
...
export const bulkAddBooksToLibraryRequestSchema = z
  .object({
    libraryId: idSchema.openapi({ description: "ID of the library" }),
    books: z
      .array(
      ...
      .min(1)
      .max(100)
      .openapi({ description: "List of books to add" }),
  })
  .openapi({ description: "Bulk add multiple books to library" });

export const transferBookRequestSchema = z
  .object({
...
export const bookImportRequestSchema = z
  .object({
    ...
    data: z
      .union([
        ...
        z.object({
          type: z.literal("json"),
          books: z
            .array(createBookRequestSchema)
            .min(1)
            .max(100)
            .openapi({ description: "Array of book objects" }),
        }),
      ])
      .openapi({ description: "Import data based on source type" }),
    defaultCondition: bookConditionEnum.default("good"),
    defaultShelfLocation: z
      .string()
      .max(100)
      .optional()
      .openapi({ description: "Default shelf location for imported books" }),
  })
