...
describe("Password Security", () => {
  ...
  describe("Password Strength Analysis", () => {
    it("should identify various attack vectors", () => {
      ...
      for (const { password, expectedIssues } of testCases) {
        ...
        const hasExpectedIssues = expectedIssues.some((issue) =>
          result.feedback.some((feedback) => feedback.toLowerCase().includes(issue.toLowerCase())),
        );
    ...
    it("should validate character complexity requirements", () => {
      ...
      for (const { password, missing } of testCases) {
        const result = evaluatePasswordStrength(password);
        expect(result.isValid).toBe(false);
        expect(result.feedback.some((fb) => fb.toLowerCase().includes(missing))).toBe(true);
      }
    });
    ...
    it("should detect patterns and sequences", () => {
      const patternPasswords = ["abc123ABC!", "123abc!@#", "qwerty123", "Password1234567890", "aaabbbccc111"];
  ...
  describe("Security Edge Cases", () => {
    ...
    it("should handle malformed hash inputs", async () => {
      const password = "TestPassword123!";
      const malformedHashes = ["", "not-a-hash", "$2b$invalid", "$2b$12$invalidhash", "random-string-123"];
    ...
    it("should handle unicode and special characters", async () => {
      const unicodePasswords = ["П@ssw0rd123!", "密码123!", "🔒secure123!", "café_münü"];
